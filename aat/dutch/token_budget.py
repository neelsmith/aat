...
Parallel to aat.english.token_budget -- same design, same reasoning,
applied to aat.dutch.dspy_signatures.analyze instead; ported wholesale
rather than re-derived, since the truncation problem this solves has
nothing to do with which language is being analyzed.

Background: `aat.dutch.dspy_signatures.analyze` (a `dspy.ChainOfThought`)
...
passage is -- and, since a sentence-clustering feature analogous to
aat.english's (aat.dutch.sentences, once a corpus-level pipeline uses it)
can combine several citation units into one passage, "how long" isn't
bounded by a single citation unit's own length either -- so any single
hard-coded `max_tokens` is eventually wrong for either a short passage
(wastes budget) or a long/complex one (truncates mid-output). This is
exactly what a real incident hit on the English side of this codebase: a
multi-unit sentence group's `analyze()` call came back truncated, and the
resulting `AdapterParseError` reported `max_tokens=None` -- which should
never be possible for a real budget. That `None` had two separate causes,
both fixed as part of adding aat.english.token_budget, and avoided here
from the start by following the same pattern:
...
2. `analyze_with_retry()` wraps `aat.dutch.dspy_signatures.analyze` and,
...
module (inherited unchanged from aat.english.token_budget, for the same
reason): arsgrammatica's SentenceAnalysis signature produces one
`TokenAnalysis` entry per INPUT token (including implied/elided ones),
so "does the output cover every input token id?" is a reliable,
LM-independent truncation signal there regardless of `finish_reason`.
AgentActionTarget's `nodes` output has no such contract, in Dutch any
more than in English -- a short passage with no agent/action/target
content at all can correctly produce zero nodes, and even a fully
analyzed passage normally covers only some of its tokens (see
AgentActionTarget's own docstring) -- so "how many nodes came back" says
nothing reliable about whether the response was cut off. This module
therefore has no `_missing_token_ids()`-equivalent primary signal;
truncation detection here leans on the two signals that ARE
schema-agnostic:
...
# reason aat.english.token_budget (and, upstream of that,
# arsgrammatica.token_budget) does this: everything else in this module
# (DEFAULT_CEILING, get_calibration(), estimate_max_tokens(),
# _looks_truncated()) is usable without a live dspy at all, so only the
# two names that actually need one (_finish_reason_was_length(),
# analyze_with_retry()) should force the import -- and importing
# aat.dutch already pulls this module in via its own __init__.py, so
# gating it narrowly here matters for the same reason it does for
# aat.core.html.tokens_to_html not needing dspy either (e.g. a WASM/
# Pyodide notebook that only reads already-saved analyses).
# `analyze` is kept as a real module-level name either way (not deferred
# into the one function that uses it) specifically so
# tests/test_dutch_token_budget.py's own
# monkeypatch.setattr("aat.dutch.token_budget.analyze", ...) calls keep
# working unchanged whenever dspy *is* installed -- which is every test
# run today, since aat's own `dev` extra depends on both `english` and
# `dutch`.
try:
    import dspy
...
# A Dutch counterpart to utilities/calibrate_max_tokens.py would write
# its fitted (intercept, slope) here, the same way that script writes
# aat/english/token_budget_calibration.json -- no such script exists yet
# (it needs a Dutch GOLD_EXAMPLES-equivalent fixture to calibrate
# against, which doesn't exist yet either -- see notes/CLAUDE_WORKFLOW.md).
# Until one is written and run, estimate_max_tokens() below falls back to
# the same conservative, deliberately-generous constants
# aat.english.token_budget started with.
CALIBRATION_FILE = Path(__file__).with_name("token_budget_calibration.json")

# Untuned stand-ins, copied from aat.english.token_budget's own fallback
# fit (see that module's own comment for the reasoning) rather than
# re-derived, since there's no Dutch calibration run yet to measure
# against and the underlying shape of the problem -- a ChainOfThought
# `reasoning` field plus a small, five-field-per-node JSON `nodes` list
# -- doesn't depend on which language is being analyzed. Deliberately
...
# truncation this module exists to avoid. Revisit once a real Dutch
# calibration run exists (see CALIBRATION_FILE's own comment above).
...
def analyze_with_retry(
    ...
    *,
    ...
    """Call `aat.dutch.dspy_signatures.analyze`, detecting truncation and
