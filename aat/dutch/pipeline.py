"""
Orchestrates aat.dutch's tokenize -> analyze -> validate pipeline: the
convenience layer most callers should use instead of calling
tokenize.py/dspy_signatures.py directly. Parallel to aat.english.pipeline
-- identical shape and logic (this whole pipeline is language-agnostic
apart from which language's tokenize()/dspy_signatures.analyze() it
calls), ported wholesale rather than re-derived.
"""
...
def analyze_units_by_sentence(
    ...
    aat.dutch.sentences's own module docstring for why that matters and
    worked examples.

    Groups `units` into sentences (aat.dutch.sentences.
    cluster_sentences()) and analyzes each group as ONE combined passage
    (aat.dutch.sentences.tokenize_units() builds its combined context/
    ...
    independently the way analyze_passages() does.
    ...
    needs no LM and no re-run of aat.dutch.sentences.
