...
- Docs are entirely manual, on purpose (see notes/CLAUDE_WORKFLOW.md): run `python utilities/build_docs.py` for the pdoc API pages, and separately `quarto render` inside `quarto/` whenever you want to refresh the hand-written site, then commit and push `docs/` yourself. There's no CI step rebuilding either on push -- the two build systems write into the same `docs/` directory and `build_docs.py` wipes it first, so run `quarto render` again afterward if its run followed a Quarto publish.
